import ssl, smtplib, mimetypes, re, getpass
...
with smtplib.SMTP_SSL("smtp.gmail.com", PORT, context=context) as server:
	while True:
		...
		senders_email = input("Enter the sender's gmail email('' to exit): ").lower()
		if senders_email=='':
			break
		while not check_email(senders_email):
			senders_email = input("Enter the sender's gmail email correctly('' to exit): ").lower()
		password = getpass.getpass(prompt="Enter the serder's gmail password: ")
		try:
		...
		recivers_email = input("Enter the reciever(s) email. Seperate be comma(',')('' to exit): ").lower()
		if recivers_email=='':
			break
		while not all(map(lambda x: check_email(x), recivers_email.split(","))):
			recivers_email = input("Enter the reciever(s) email correctly. Seperate be comma(',')('' to exit): ")
		...
		Attachement = input("\nDo you want to add attachement?(Y/N): ").strip().lower()
		if Attachement == "y":
